...
const enter = document.getElementById("confirm");
const Ndomanda = document.getElementById("Ndomanda");

const currentUser = localStorage.getItem("currentUser");
const currentDifficulty = localStorage.getItem("currentDifficulty") || "Easy"; //in questo caso l'operatore || serve a dare un valore di default in caso dal local storage non torni nulla
...
// N domande in base a localStorage (salvato in index.js)
let N = Number(localStorage.getItem("N"));
if (!N || N > questions.length) {
  N = questions.length;
}
...
// mischai l'array delle risposte in modo che a schermo vengano stampate sempre in modo diverso
function shuffleArray(array) {
  return array.sort(() => Math.random() - 0.5);
}

// crea in html la struttura per le risposte
function createAnswer(text, isCorrect, index, answers) {
  const label = document.createElement("label");
  label.style.display = "block";
...
function showQuestion() {
  if (index >= N || index >= questions.length) {
    nextQuestion();
    return;
  }
  ...
  qText.textContent = currentQuestion.question;
  Ndomanda.textContent = `Question ${index + 1} / ${N}`;

  let answers = [];

  createAnswer(currentQuestion.correct_answer, true, index, answers);
  currentQuestion.incorrect_answers.forEach((ans) => createAnswer(ans, false, index, answers));
...
function nextQuestion() {
  ...
  if (index < N) {
    ...
    if (currentUser) {
      saveQuizResult(currentUser, points, N, currentDifficulty);
    }
    window.location.href = "results.html";
  }
}

// pulsante conferma risposta
enter.addEventListener("click", () => {
...
function saveQuizResult(username, score, totalQuestions, difficulty) {
